...
# Usage :
########## Proxmox VE 8
# installimage-proxmox.sh hostname.fqdn.com
########## Proxmox VE 9
# installimage-proxmox.sh hostname.fqdn.com pve9
########## Backup Server
# installimage-proxmox.sh hostname.fqdn.com pbs
#
###############################################################################
# Assumptions:
...
DEBIAN_CODENAME="${MY_DEBIAN_CODENAME}"
...
installimage_file="/root/images/Debian-${DEBIAN_CODENAME}-latest-amd64-base.tar.gz"
# Detect the latest installimage file to use
#installimage_file=$(find /root/images/ -iname "Debian-*-${DEBIAN_CODENAME}-amd64-base.tar.gz" | sort --version-sort --field-separator=- --key=2,2 -r | head -n1)
...
#### CONFIGURE ZFS L2ARC PARTITION SIZE&
if [ "$MY_ZFS_L2ARC" != "" ] ; then
  ZFS_L2ARC="${MY_ZFS_L2ARC}"
