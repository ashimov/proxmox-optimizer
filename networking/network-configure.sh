...
# XS_DHCP_PUBLIC="yes" to enable DHCP on vmbr0 (public bridge).
#
################################################################################
...
if [ -z "$XS_DHCP_PUBLIC" ] ; then
  XS_DHCP_PUBLIC="no"
fi
dhcp_interfaces="vmbr1"
if [ "${XS_DHCP_PUBLIC,,}" == "yes" ] ; then
  dhcp_interfaces="vmbr0 vmbr1"
fi
...
if ! [ -f "network-addiprange.sh" ]; then
  ...
  curl -O https://raw.githubusercontent.com/ashimov/proxmox-optimizer/master/networking/network-addiprange.sh && chmod +x network-addiprange.sh
fi
