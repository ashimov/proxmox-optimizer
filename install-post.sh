...
################################################################################
#
# Tested on Proxmox Version: 8.x / 9.x
#
...
# are set in the install-post.env, see the sample : install-post.env.sample
...
if [ "${XS_CEPH,,}" == "yes" ] ; then
    # Add the latest ceph provided by proxmox
    # Proxmox 8: ceph-reef, Proxmox 9: ceph-squid
    PROXMOX_VERSION=$(pveversion | grep -oP 'pve-manager/\K[0-9]+' | head -1)
    if [ "$PROXMOX_VERSION" -ge 9 ] 2>/dev/null; then
        CEPH_RELEASE="squid"
    elif [ "$PROXMOX_VERSION" -ge 8 ] 2>/dev/null; then
        CEPH_RELEASE="reef"
    else
        CEPH_RELEASE="quincy"
    fi
    echo "deb https://download.proxmox.com/debian/ceph-${CEPH_RELEASE} ${OS_CODENAME} main" > /etc/apt/sources.list.d/ceph.list
    ## Refresh the package lists
    apt-get update > /dev/null 2>&1
...
if [ "${XS_AMDFIXES,,}" == "yes" ] ; then
    ...
    if [ "${XS_AMDFIXES,,}" == "yes" ] ; then
      #Apply fix to kernel : Fixes random crashing and instability
        ...
        PROXMOX_VERSION=$(pveversion | grep -oP 'pve-manager/\K[0-9]+' | head -1)
        if [ "$PROXMOX_VERSION" -ge 9 ] 2>/dev/null; then
            echo "Installing latest kernel for Proxmox 9"
            /usr/bin/env DEBIAN_FRONTEND=noninteractive apt-get -y -o Dpkg::Options::='--force-confdef' install pve-kernel-6.11
        elif [ "$PROXMOX_VERSION" -ge 8 ] 2>/dev/null; then
            echo "Installing latest kernel for Proxmox 8"
...
# net.ipv4.tcp_tw_recycle removed - deprecated since kernel 4.12
...
echo "# ashimov.com" > /etc/ashimov
date >> /etc/ashimov

## Script Finish
echo -e '\033[1;33m Finished....please restart the system \033[0m'
echo "Optimisations by https://github.com/ashimov/proxmox-optimizer"
